...
class MarketDataService {
    ...
    func fetchCreditRates() async throws -> [CreditOffer] {
        ...
        func fetchStocks() async throws -> [Stock] {
            ...
            guard let url = URL(string: "https://api.collectapi.com/economy/hisseSenedi") else {
            ...
            // Veriyi çevirmeden önce konsola ham halini yazdırıyoruz:
            if let jsonString = String(data: data, encoding: .utf8) {
                print("API'den Gelen Ham Veri: \(jsonString)")
            }
                    
            // Eski kodun aynen burada kalacak:
